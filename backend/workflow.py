...
from backend.agents.fact_anchor import fact_anchor_agent
from backend.agents.parallel_analysts import (
...
from backend.agents.consistency_checker import consistency_checker_agent, generate_final_report
...
def _safe_print(*args: object, **kwargs: object) -> None:
    """
    Windows + uvicorn 某些场景下 stdout 可能是已关闭的管道，会导致 print 抛 OSError (WinError 233)。
    这里做兜底：日志打印失败不应影响工作流主流程。
    """
    try:
        print(*args, **kwargs)
        try:
        ...
        # e.g. [WinError 233] 管道的另一端上无任何进程
        return


def fact_anchor_node(state: WorkflowState) -> WorkflowState:
    """阶段一：事实锚定节点"""
    _safe_print("阶段一：事实锚定 Agent 正在工作...")
...
def consistency_check_node(state: WorkflowState) -> WorkflowState:
    """阶段三：逻辑交叉校验节点"""
    _safe_print("阶段三：逻辑交叉校验 Agent 正在工作...")
...
def retry_node(state: WorkflowState) -> WorkflowState:
    """重试节点：回退到并行分析阶段"""
    _safe_print(f"检测到逻辑不一致，进行第 {state['retry_count'] + 1} 次重试...")
    _safe_print(f"发现的问题：{state['consistency_issues']}")
...
def force_generate_node(state: WorkflowState) -> WorkflowState:
    ...
    _safe_print("已达到最大重试次数，强制生成最终报告...")
    ...
    try:
        ...
        if final_report:
            ...
            if final_state.get("parallel_results") and final_state.get("fact_anchor_result"):
                from backend.agents.consistency_checker import generate_final_report
